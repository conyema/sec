...
describe('Estate controller tests:', () => {
  ...
  //   it('should send appropriate response if estate does not exist', async () => {
  //     const req = {};
  //     const res = { status: sinon.spy(), json: sinon.spy() };
  //     const next = sinon.spy();
  ...
  //     await controller.getOneEstates(req, res, next);

  //     // assertions for empty response
  //     expect(next.calledOnce).to.equal(true);
  //     expect(next.args[0][0].status).to.equal(404);
  //     expect(next.args[0][0].message).to.equal('Estate not found');
  //   });
